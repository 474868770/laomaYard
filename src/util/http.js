...
axios.defaults.baseURL = 'http://api.growbb.net:3009/api/'; //这是调用数据接口

// http request 拦截器，通过这个，我们就可以把Cookie传到后台
axios.interceptors.request.use(
  config => {
    const token = getCookie(); //获取Cookie
    config.headers = {
      'Content-Type':'application/json' //设置跨域头部
    };
    if (token) {
      config.headers['Authorization'] = 'JWT' + getToken()//后台接收的参数，后面我们将说明后台如何接收
    }
    return config;
  },
  err => {
